...
class AppStore extends Component {
  state = {tab1Id: '', searchInput: ''}

  changeStateOfTab = tabId => {
    this.setState({tab1Id: tabId})
  }
  ...
  render() {
    const {tab1Id, searchInput} = this.state
    const itemList = appsList.filter(each => tab1Id === each.category)
    const lll = tab1Id === '' ? appsList : itemList

    const ggg = lll.filter(each =>
      ...
      <div className="mainContainer">
        ...
        <ul className="listItem">
          {tabsList.map(each => (
            <TabItem
              tabDetails={each}
              Active={tab1Id === each.tabId}
              key={each.tabId}
              changeStateOfTab={this.changeStateOfTab}
            />
          ))}
        </ul>
        <ul className="appListContainer">
          {ggg.map(each => (
            <AppItem listDeatil={each} key={each.appId} />
          ))}
        </ul>
      </div>
    )
